...
const updatePastrySchema = Joi.object({
    ...
    maxWeight: Joi.number().positive().greater(Joi.ref('minWeight')).messages({
...
export const validateUpdatePastry = (req: Request, res: Response, next: NextFunction) => {
    const { error } = updatePastrySchema.validate(req.body, { abortEarly: false });
